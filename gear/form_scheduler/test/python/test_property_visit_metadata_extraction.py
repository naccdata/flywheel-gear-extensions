...
def visit_metadata_strategy(draw):
    ...
    return {
        ...
        "date": draw(st.dates(min_value=datetime(2020, 1, 1).date(), max_value=datetime(2024, 12, 31).date())).strftime("%Y-%m-%d"),
...
def test_visit_metadata_extraction_priority_custom_info_first(
    ...
    """Property test: When both QC status custom info and JSON metadata are available,
    custom info should be used first.
...
@settings(max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture])
def test_visit_metadata_extraction_fallback_to_json(
    event_accumulator, json_data
):
    """Property test: When QC status custom info is not available,
    should fall back to JSON file metadata.
...
@settings(max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture])
def test_visit_metadata_extraction_no_qc_file(
    event_accumulator, visit_data
):
    """Property test: When QC status file is None,
    should fall back to JSON file metadata.
...
def test_visit_metadata_extraction_invalid_data(event_accumulator):
    ...
    **Validates: Requirements 2.3, 2.4, 4.1, 4.2, 4.3**
    ...
    json_file = create_mock_file_entry(
        "forms.json",
        {"forms": {"json": {"ptid": None, "visitdate": None, "module": None}}}  # Invalid
    )
