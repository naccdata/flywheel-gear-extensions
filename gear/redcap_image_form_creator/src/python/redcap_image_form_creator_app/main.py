...
def tag_pass(session: ContainerOutput):
...
def tag_fail(dry_run: bool, session: ContainerOutput, msg: str):
    if not dry_run:
...
def find_flywheel_origin_user_id(
    dry_run: bool, session: ContainerOutput, flywheel_obj, proxy: FlywheelProxy
):
...
def get_record_id_suffix(session: ContainerOutput, proxy: FlywheelProxy):
