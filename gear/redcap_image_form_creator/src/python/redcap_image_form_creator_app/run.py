...
class REDCapImageFormCreatorVisitor(GearExecutionEnvironment):
    ...
    def create(
        ...
        try:
            ...
            raise GearExecutionError(f'Incomplete configuration: {error.message}') from error
    ...
    def run(self, context: GearContext) -> None:
        ...
        if context.config.destination['type'] == 'session':
            ...
            session_id = self.proxy.get_container_by_id(context.config.destination['id']).parents['session']
        else:
        ...
        redcap_con = REDCapConnection.create_from(
            self.__param_store.get_parameters(
                ...
                parameter_path=self.__parameter_path
            )
        )
