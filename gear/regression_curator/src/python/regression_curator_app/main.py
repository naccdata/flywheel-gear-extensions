...
def process_header(header_line: str, keep_fields: List[str]) -> List[str]:
    ...
    if missing:
        raise GearExecutionError(f'Required fields not found in QAF header: {missing}') 
...
def localize_qaf(s3_qaf_file: str, keep_fields: List[str]) -> MutableMapping:
    ...
    fields specified by the keep_fields parameter. Assumes
    no case-sensitivity and converts headers to lowercase.
    ...
    for line, row in enumerate(body.iter_lines()):
        ...
        if not header:
            header = process_header(row)
            continue

        # convert row to dict
        row = row.split(',')
        if len(header) != len(row):
            raise GearExecutionError(f"Malformed CSV row on line {line}")
