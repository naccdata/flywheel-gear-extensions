"""Test that web_access_level and adrc_reports_access_level are handled correctly in to_user_entry()."""
...
def test_web_access_not_in_authorizations():
    """Test that web_access_level is not included in user authorizations.
    
    The web_access_level field is for general webinars/presentations access
    and doesn't map to a study-specific datatype, so it's not included in
    the authorizations list.
    """
    ...
    # web_access_level should not create any authorizations
    # Only the P30 enrollment and form access should be present
    assert len(user_entry.authorizations) == 1
    assert user_entry.authorizations[0].study_id == "adrc"
    
    # Should have enrollment and form datatypes from P30 fields
    activities = user_entry.authorizations[0].activities
    assert "enrollment" in activities
    assert "form" in activities


def test_adrc_reports_not_in_authorizations():
    """Test that adrc_reports_access_level is not included in user authorizations.
    
    The adrc_reports_access_level field is for ADRC dashboards/reports access
    but 'reports' is not a valid datatype in DatatypeNameType, so it's ignored
    by the authorization parsing logic.
    """
    ...
    # adrc_reports_access_level should not create any authorizations
    # Only the P30 enrollment and form access should be present
    assert len(user_entry.authorizations) == 1
    assert user_entry.authorizations[0].study_id == "adrc"
