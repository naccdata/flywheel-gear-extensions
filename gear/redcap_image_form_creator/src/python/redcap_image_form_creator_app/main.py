...
def inspect_acquisitions(
        dry_run: bool,
        session: SessionOutput,
        session_info_to_import: dict,
        proxy: FlywheelProxy
):

    def set_or_agree(key_to_set: str, val_to_set: str, info_context: str):
        if key_to_set not in session_info_to_import:
            ...
            tag_fail(
                ...
                'for {key_to_set}from {info_context}'
            )
    fw_mri_series = []
...
def construct_session_info_for_redcap(
    ...
    if not isinstance(adcid, int):
        tag_fail(
            dry_run,
            session,
            f'Expected adcid to be int, not {type(adcid)} for {adcid}'
       )
    ...
    if 'uploader_email' in session_info_to_import:
        ...
        if user is None:
            ...
            session_info_to_import['uploader_fullname'] = user.firstname + ' ' + user.lastname
    else:
        tag_fail(dry_run, session, 'Missing uploader_email after inspecting acquisitions')
...
def get_record_id_suffix(
    ...
    for proj_ses in fw_project.sessions():
        if proj_ses.id == session.id:
            continue
        ses = proxy.get_container_by_id(proj_ses.id) # session's custom info not populating if grabbed directly from fw_project.sessions()
        ses.reload()
        if 'record_id' in ses.info:
            ...
            if record_id_pattern.match(existing_record_id):
                ...
                log.info(f'  Ignoring conconforming record_id {existing_record_id}')
...
def compose_record_id(
    ...
    composed_id = 'IMG' + str(adcid).zfill(2) + '_' \
        ...
        for proj_ses in fw_project.sessions():
            ...
            if 'record_id' in ses.info:
                if ses.info['record_id'] == record_id:
                    log.info(f'Conflict during pass {i+1}/{MAX_NEW_RECORD_ATTEMPTS} in record_id {record_id} for session {ses.subject.label}::{ses.label} (ses.id)')
                    if int(ses.id, 16) < int(session.id, 16):
                        ...
                        for p in range(MAX_PAUSES_FOR_OTHER_SESSION):
                            log.info(f'Waiting to see if {ses.id} will yield ' \
                                     f'{record_id} ({p+1}/{MAX_PAUSES_FOR_OTHER_SESSION})')
                            sleep(1.0)
                            ses = proxy.get_container_by_id(ses.id) # reload() does not update ses.info['record_id']
                            if ses.info['record_id'] != record_id:
...
def import_new_record_for_session(
    ...
    record_id = generate_unique_record_id(dry_run, session, proxy, session_info_to_import['adcid'])
    ...
    log.info(f'Connected to REDCapProject with pid {redcap_proj.pid} '
             f'and title {redcap_proj.title}')

    if record_id:
        if dry_run:
            log.info(
                'Dry run -- skipping tag and import to REDCap ' \
                f'{session_info_to_import}'
                )
        else:
        ...
        tag_fail(
            dry_run,
            session,
            'Unable to generate unique new record_id'
        )
...
def run(
    ...
    if session.container_type != 'session':
        ...
        if session.container_type != 'session':
            ...
            session = proxy.get_container_by_id(session.parents[0])
            if session.container_type != 'session':
                raise GearExecutionError(f'Expected session, not {session.container_type}')
    if 'record_id' in session.info:
        if session.info['record_id']:
