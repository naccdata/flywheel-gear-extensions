...
def valid_visit_metadata_strategy(draw):
    ...
    return {
        "ptid": draw(st.text(min_size=1, max_size=10, alphabet=st.characters(whitelist_categories=("Nd", "Lu")))),
        "date": draw(st.dates().map(lambda d: d.strftime("%Y-%m-%d"))),
        "module": draw(st.sampled_from(["UDS", "LBD", "FTLD", "MDS"])),
        "visitnum": draw(st.one_of(st.none(), st.text(min_size=1, max_size=3, alphabet=st.characters(whitelist_categories=("Nd",))))),
...
def invalid_visit_metadata_strategy(draw):
    ...
    base_data = {
        ...
        "date": draw(st.one_of(st.none(), st.dates().map(lambda d: d.strftime("%Y-%m-%d")))),
        "module": draw(st.one_of(st.none(), st.sampled_from(["UDS", "LBD", "FTLD", "MDS"]))),
...
def test_valid_visit_metadata_passes_validation(valid_metadata):
    """Property test: For any VisitMetadata with required fields (ptid, date, module),
    validation should pass and allow VisitEvent creation.
...
def test_invalid_visit_metadata_fails_validation(invalid_metadata):
    """Property test: For any VisitMetadata missing required fields (ptid, date, module),
    validation should fail and skip event logging.
...
def test_visit_metadata_extraction_validation_integration(event_accumulator):
    ...
    **Validates: Requirements 4.4, 4.5**
    ...
    qc_log_invalid = create_mock_file_entry(
        "qc-status.json",
        {"visit": {"ptid": None, "date": "2024-01-15", "module": "UDS"}}  # Missing ptid
    )
    ...
    qc_log_invalid = create_mock_file_entry(
        "qc-status.json",
        {"visit": {"ptid": None, "date": None, "module": None}}
    )
    ...
    result = event_accumulator._extract_visit_metadata(json_file_invalid, qc_log_invalid)
...
def test_visit_metadata_validation_with_optional_fields():
    ...
    **Validates: Requirements 4.4, 4.5**
    ...
    minimal_metadata = VisitMetadata(
        ptid="110001",
        date="2024-01-15", 
        module="UDS"
    )
...
@given(
    ...
    date=st.one_of(st.none(), st.text(max_size=0), st.dates().map(lambda d: d.strftime("%Y-%m-%d"))),
    module=st.one_of(st.none(), st.text(max_size=0), st.sampled_from(["UDS", "LBD", "FTLD", "MDS"]))
)
@settings(max_examples=100)
