...
class CSVTransformVisitor(CSVVisitor):
    ...
    def visit_row(self, row: Dict[str, Any], line_num: int) -> bool:
        ...
        self.__add_to_current_batch(
            subject_lbl=subject_lbl, input_record=transformed_row
        )
        return True
                ...
                if (
                    ...
                    if visit_num not in prev_visit_nums:
                        ...
                        log.error(
                            ...
                            transformed_row[FieldNames.PTID],
                            visitdate,
                            visit_num,
                        )
                        continue
    ...
    def __copy_downstream_gears_metadata(
        ...
        *,
        ...
        if self.__project is None:
            log.warning("Parent project not specified to update visit error log")
            return False
        if not self.module:
        ...
        if not error_log_file:
            log.error(
                "Failed to retrieve visit error log file %s/%s",
        ...
        info.set_errors(
            gear_name=self.__gear_name,
            status=gear_state.upper(),  # type: ignore
            errors=self.__error_writer.errors(),
        )

        try:
            error_log_file.update_info(info.model_dump(by_alias=True))
...
def run(
    *,
    ...
    result = (
        visitor.update_existing_visits_error_log(downstream_gears=downstream_gears)
        and result
    )
