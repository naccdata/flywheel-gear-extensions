...
def parameter_store(fake_ssm):
    ...
    ssm.put_parameter(
        Name="/path/to/another/key/fifth-key", Value="robinson", Type="String"
    )
...
def test_extract_parameter_returns_key_pair_tuple(parameter_store):
    parameter = {"Name": "key", "Value": "hello", "Type": "SecureString"}
...
def test_get_parameters_by_path_without_recursion(parameter_store):
    parameter_keys = parameter_store.get_parameters_by_path(
        "/path/to/", recursive=False
    )
    assert len(parameter_keys) == 2


def test_stripping_path_in_parameter(parameter_store):
    parameter_keys = parameter_store.get_parameter(
        "/path/to/third-key", strip_path=False
    )
    assert parameter_keys.get("/path/to/third-key")

    stripped_parameter_keys = parameter_store.get_parameter(
        "/path/to/third-key", strip_path=True
    )
...
def test_get_parameters_with_hierarchy(parameter_store):
    ...
    parameter_keys = parameter_store.get_parameters_with_hierarchy(
        "/path/to", strip_path=True
    )
    assert parameter_keys == expected_result

    parameter_keys = parameter_store.get_parameters_with_hierarchy(
        "/path/to", strip_path=False
    )
...
def test_get_parameters_with_hierarchy_for_path_with_no_nesting(parameter_store):
    parameter_keys = parameter_store.get_parameters_with_hierarchy(
        "/path/to/another/key/", strip_path=True
    )
...
def test_get_parameters_with_hierarchy_for_nonexistent_path(parameter_store):
    parameter_keys = parameter_store.get_parameters_with_hierarchy(
        "/does/not/exist", strip_path=True
    )
