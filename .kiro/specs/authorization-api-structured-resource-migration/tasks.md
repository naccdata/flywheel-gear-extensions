...
- [ ] 1. Correct `ResourceObject` in place to match the API schema
  - [ ] 1.1 Rework `ResourceObject` fields and request serialization in `common/src/python/authorization/models.py`
  ...
  - [ ] 1.2 Add the per-type parent-field `check_parent_fields` model validator to `ResourceObject`
  ...
  - [ ] 1.3 Write property test for per-type parent-field validation in `common/test/python/authorization_test/test_property_models.py`
...
- [ ] 2. Migrate write-request models and client request construction to structured `resource`
  - [ ] 2.1 Embed structured `resource` in the four write-request models in `common/src/python/authorization/models.py`
  ...
  - [ ] 2.2 Add structured fields to the `BatchOperation` domain type in `common/src/python/authorization/models.py`
  ...
  - [ ] 2.3 Update `grant`/`revoke`/`check_permission` request construction in `common/src/python/authorization/client.py`
  ...
  - [ ] 2.4 Update batch request construction and idempotent 409/404 result construction in `common/src/python/authorization/client.py`
  ...
  - [ ] 2.5 Write property test that requests carry structured identity and never flat identity in `common/test/python/authorization_test/test_property_grant_revoke.py`
  ...
  - [ ] 2.7 Update/extend client unit tests for structured requests and idempotency in `common/test/python/authorization_test/test_client_grant.py`, `test_client_revoke.py`, `test_client_batch.py`
...
- [ ] 4. Carry structured identity through `DesiredGrant` and the translator
  - [ ] 4.1 Rework `DesiredGrant` to carry the Structured Identity in `common/src/python/authorization_sync/models.py`
  ...
  - [ ] 4.2 Populate structured fields in `translate()` in `common/src/python/authorization_sync/translator.py`
  ...
  - [ ] 4.3 Tighten `build_label_for_resource_prefix` unknown-prefix handling in `common/src/python/authorization_sync/resource_ids.py`
  ...
  - [ ] 4.5 Update/extend translator unit tests for structured `DesiredGrant` in `common/test/python/authorization_sync_test/test_translator.py`
...
- [ ] 5. Parse responses from the structured `resource`
  - [ ] 5.1 Read identity from `resource` in the response models in `common/src/python/authorization/models.py`
  ...
  - [ ] 5.2 Update `UserPermissions.to_grants` factory contract in `common/src/python/authorization/models.py`
  ...
  - [ ] 5.4 Update response-parsing unit tests to read from structured `resource` in `common/test/python/authorization_test/test_client_query.py`, `test_property_models.py`
...
- [ ] 6. Key the diff on the Structured Identity Tuple in the sync service
  - [ ] 6.1 Build `DesiredGrant` from `entry.resource` in `_grants_in_scope` and the `to_grants` factory in `common/src/python/authorization_sync/sync_service.py`
  ...
  - [ ] 6.2 Preserve scope-aware revocation in `_entry_in_scope` in `common/src/python/authorization_sync/sync_service.py`
  ...
  - [ ] 6.3 Write property test that the diff round-trips on structured identity independent of `flat_id` in `common/test/python/authorization_sync_test/test_multi_study_sync.py`
  ...
  - [ ] 6.4 Write property test that a None-resource entry is never revoked in `common/test/python/authorization_sync_test/test_multi_study_sync.py`
  ...
  - [ ] 6.5 Write property test that community-scoped entries are never revoked in `common/test/python/authorization_sync_test/test_multi_study_sync.py`
  ...
  - [ ] 6.6 Write property test that revoke candidacy is center-keyed in `common/test/python/authorization_sync_test/test_multi_study_sync.py`
  ...
  - [ ] 6.7 Update sync-service diff unit tests to key on the Structured Identity Tuple in `common/test/python/authorization_sync_test/test_fault_isolation.py`, `test_pipeline_integration.py`, `test_sync_profile.py`
...
- [ ] 8. Dispose of `build_resource_id` and reconcile the seed path
  - [ ] 8.1 Confirm `build_resource_id` retention and remove it from the grant path
