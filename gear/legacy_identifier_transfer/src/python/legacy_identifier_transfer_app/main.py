...
def validate_and_create_record(
    ...
    if naccid != identifier.naccid:
        log.error("NACCID mismatch: key %s != value %s", naccid, identifier.naccid)
        return None
...
def process_record_collection(
    ...
    log.info("Trying to import %d legacy enrollment records", len(record_collection))
    ...
    for record in record_collection:
        ...
        if dry_run:
            log.info("Dry run: would create enrollment for subject %s", record.naccid)
            success_count += 1
            continue
    ...
    log.info("Successfully imported %d legacy enrollment records", success_count)
...
def get_enrollment_date(subject_id: str, forms_store: FormsStore) -> Optional[datetime]:
    ...
    try:
        enroll_date = parse_date(
            date_string=ivp_packet[date_col_lbl], formats=[DEFAULT_DATE_FORMAT]
        )
        return enroll_date
    except DateFormatException:
        log.error(
            "Unable to parse initial visit date %s for subject %s",
            ivp_packet[date_col_lbl],
            subject_id,
        )
        return None
...
def process_legacy_identifiers(  # noqa: C901
    ...
    for naccid, identifier in identifiers.items():
        try:
            ...
            record = validate_and_create_record(naccid, identifier, enrollment_date)
            if record:
...
def send_email(
    ...
    body = (
        f"Failed to transfer {failed_count} legacy identifiers "
