...
def reset_upload_checkbox(
    ...
    for visit in visits:
        update = {}
        update[redcap_prj.primary_key_field] = visit[redcap_prj.primary_key_field]
...
def validate_redcap_report(
    ...
    if redcap_prj.has_repeating_instruments_or_events() and (
        "redcap_event_name" not in record or "redcap_repeat_instance" not in record
    ):
        raise GearExecutionError(
...
def run(
    *,
    ...
    try:
        ...
        raise GearExecutionError(
            f"Failed to read schema file {schema_file}: {error}"
        ) from error
    ...
    try:
        ...
        if isinstance(redcap_con, REDCapReportConnection):
            ...
            extra_fields = validate_redcap_report(
                redcap_prj, redcap_con.report_id, records_list[0], schema["definitions"]
            )
        ...
        # For longitudinal projects assumes the events are defined by module
            ...
            records_list = redcap_prj.export_records(
                fields=fields, events=events, filters=filters
            )  # type: ignore
    except REDCapConnectionError as error:
        raise GearExecutionError(error.message) from error
